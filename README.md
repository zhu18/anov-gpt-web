...
![cover](./docs/ANOV-GPT.gif)

## 介绍

这是一个可以自己在本地部署的`ANOV-GPT`网页，使用`chatGLM2-6B`模型来实现接近`ChatGPT`的对话效果。
源代码Fork和修改于[zhu18/anov-gpt-web](https://github.com/zhu18/anov-gpt-web/) 

与`ChatGPT`对比，`ANOV-GPT Web`有以下优势：

1. **独立部署**。`ANOV-GPT Web`只需要一个能运行`chatGLM2-6B`模型的服务器即可使用，可以使用自己微调的GLM模型。
2. **完全离线**。`ANOV-GPT Web`依赖于`chatGLM2-6B`模型，可以在离线环境或者内网中使用。

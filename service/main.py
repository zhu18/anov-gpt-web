...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Simple API server for ANOV-GPT-6B')
    ...
    # model_name = "THUDM/ANOV-GPT-6b"
    quantize = int(args.quantize)
    model = None
    if args.device == 'cpu':
        model_path = model_path if  model_path else "THUDM/ANOV-GPT-6b-int4" 
        tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True)
        model = AutoModel.from_pretrained(model_path, trust_remote_code=True).float()
    else:
        model_path = model_path if  model_path else "THUDM/ANOV-GPT-6b" 
        tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True)
        if quantize == 16:
